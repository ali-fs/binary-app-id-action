...
class AppIdGenerator {
    authorise() {
        return new Promise(async (resolve) => {
            log('Authorising...');
            const authorise = await this.resolveApiRequest('authorize', { authorize: core.getInput('BINARY_API_TOKEN') });
    ...
    getCreatedApps() {
        return new Promise(async (resolve) => {
            ...
            apps.forEach((app) => { if (app.github) log(`> ${app.redirect_uri} (${app.app_id})`) });
    ...
    async runAction() {
        return new Promise(async (resolve, reject) => {
            ...
            const binary_app_id = Number(core.getInput('BINARY_APP_ID'));

            const open_pull_requests = await this.getOpenPullRequests();
            if (!open_pull_requests) return reject();

            const websocket = new WebSocket(`wss://frontend.binaryws.com/websockets/v3?app_id=${binary_app_id}&brand=binary&lang=EN`);

            websocket.addEventListener('error', warn);

            this.api = new DerivAPI({ app_id: binary_app_id, connection: websocket });
            ...
            const app_to_recycle = apps.find(
                (app) => app.github === issue.pull_request.html_url && app.redirect_uri !== `${preview_url}/en/logged_inws.html`
            );
            ...
            if (!app_to_recycle) {
                ...
                if (existing_app) {
                    log('There was an existing App ID for this URL. No action will be taken as a comment should have already been posted.\n');
            ...
            // A recyclable app is a Binary App which has a link in its `github` field that isn't
            ...
            const app_options = {
                ...
                redirect_uri: `${preview_url}/en/logged_inws.html`,
                verification_uri: `${preview_url}/en/redirect.html`,
